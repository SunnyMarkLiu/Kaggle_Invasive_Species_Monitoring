...
def main():
    ...
    print 'built ResNet50 model'
    # build the inception v3 network
    image_input = Input(shape=input_shape)
    model = applications.ResNet50(weights='imagenet', include_top=False, input_tensor=image_input)
    print('Model loaded.')
    ...
    model = Model(input=image_input, output=top_model, name='resnet_50')
    ...
    checkpoint_lr_decay = ModelCheckpointAndLearningRateDecay(Configure.resnet50_best_model_weights,
                                                              lr_decay=0.9,
    ...
    print '============ load weights ============'
    model.load_weights(Configure.resnet50_best_model_weights)
    ...
    predict_df.to_csv(Configure.submission_path.format('resnet_50'), index=False)
    print 'done.'
